...
#params$dataset  = read_datafile(paste0(HOME, "/data/nmdatPKPD.csv"))
  ...
  study_name = "TEST-FY-001"
BODY_REPLACE_ALL_TEXT = NULL
BODY_REPLACE_ALL_TEXT["AUTHOR_NAME"] <- "Feng Yang"
BODY_REPLACE_ALL_TEXT["BOSS_NAME"] <- "John Davis"

BODY_REPLACE_ALL_TEXT["STUDY_NAME"] = "TEST-FY-001"
BODY_REPLACE_ALL_TEXT["DRUG_NAME"] = "FY001"
BODY_REPLACE_ALL_TEXT["TARGET_NAME"] <- "C5"
BODY_REPLACE_ALL_TEXT["ENDPOINT_NAME"] <- "CH50"
 

BODY_REPLACE_ALL_TEXT <- data.frame(
  OLD_VALUE= names(BODY_REPLACE_ALL_TEXT), 
  NEW_VALUE = BODY_REPLACE_ALL_TEXT, 
  stringsAsFactors = FALSE
  )
attr(BODY_REPLACE_ALL_TEXT, "title") <- "A list of text to be globally substituted by another text"
values$data[["BODY_REPLACE_ALL_TEXT"]] = BODY_REPLACE_ALL_TEXT
...
BODY_REPLACE_TEXT_AT_BKM = NULL
BODY_REPLACE_TEXT_AT_BKM["BIG_BOSS_NAME"] <- "Thomas DiCioccio"
BODY_REPLACE_TEXT_AT_BKM["REPORTING_DATE"] <- format(Sys.time(), '%b %d, %Y')
BODY_REPLACE_TEXT_AT_BKM["N_SAMPLES"] <- n.samples
BODY_REPLACE_TEXT_AT_BKM["N_SUBJECTS"] <- n.USUBJID
BODY_REPLACE_TEXT_AT_BKM["LAST_SAMPLE_DATE"] <- last.date
BODY_REPLACE_TEXT_AT_BKM <- data.frame(
  BOOKMARK=names(BODY_REPLACE_TEXT_AT_BKM), 
  VALUE = BODY_REPLACE_TEXT_AT_BKM, 
  stringsAsFactors = FALSE
  )

attr(BODY_REPLACE_TEXT_AT_BKM, "title") <- "A list of bookmark to be substituted by text"
values$data[["BODY_REPLACE_TEXT_AT_BKM"]] = BODY_REPLACE_TEXT_AT_BKM
...
attr(tabl, "title") = paste0("Summary of sampling time in ", study_name)
attr(tabl, "footnote") <- "Number of subjects with at least one quantifiable sample.  
 QW = Once a week; IV = Intravenous; SC = Subcutaneous.   A loading dose of 15 mg/kg was used in the dose group of 400 mg SC QW.   
"
values$table[["tab_summary_study_sampling_time"]] = as.data.frame(tabl)
...
attr(tabl, "title") = paste0("Summary of PK samples in ", study_name)
attr(tabl, "footnote") <- "QW = Once a week; IV = Intravenous; SC = Subcutaneous."
values$table[["tab_summary_population_samples"]] = as.data.frame(tabl)
...
tdata = adsl %>% select(WGTBL, HGTBL, AGE, BMIBL, BSABL, 
                               SEX, RACE, ETHNIC)
tabl = qwraps2::summary_table(tdata, qsummary(tdata))


tabl <-  adsl %>% calc_stats(id="USUBJID", group_by ="SEX", value="WGTBL") %>% select(N, Mean, Median, Range)
attr(tabl, "title") = paste0("Summary of Patient Demographic in ", study_name)
attr(tabl, "footnote") <- "your footnote"

values$table[["tab_summary_patient_demographic"]] = tabl
...
fig_lst <- names(output$figure)  
out_figure <- lapply(1:length(fig_lst), function(i) {
  knit_expand(paste0(HANDBOOK_HOME, '/lib/child_figure.Rmd'), 
  figure_name = fig_lst[i],             
  figure_label= gsub("_", "-", fig_lst[i], fix=TRUE), 
  figure_caption = attr(FIGURE[[fig_lst[i]]], "title")
  )
}) %>% unlist()
...
tabl_lst <- names(output$table)  
out_table <- lapply(1:length(tabl_lst), function(i) {
  knit_expand(paste0(HANDBOOK_HOME, '/lib/child_table.Rmd'), 
  table_name = tabl_lst[i],             
  table_label= gsub("_", "-", tabl_lst[i], fix=TRUE), 
  table_caption = attr(TABLE[[tabl_lst[i]]], "title")
  )
}) %>% unlist()
