  ...
  fig = ggplot(PKPD , aes(x=PK, y=PD, col=ARMA)) + 
  ...
  attr(fig, 'title') <-  "Sigmoid PK-PD (Percent Change from Baseline) Relationship Regardless of Time Points and Dose Groups [Log-Log Scale]"
  ...
  fig = fig0 + facet_wrap(~USUBJID) + #ggplot2::theme(legend.position = "none") + #, scales="free")
    geom_segment(aes(xend=c(tail(PK, n=-1), NA), yend=c(tail(PD, n=-1), NA)),
  ...
  attr(fig, 'width') <- 8
  attr(fig, 'height') <- 6
  FIGURE_ALL[["HYSTERESIS_INDIV_PCHG_LOG"]] = fig 
  ...
  fig = ggplot() +  
    ...
    scale_x_continuous(name="Time (Day)", breaks=x$breaks, label=x$labels) +
    scale_y_continuous(name=paste0("Concentration of ", PK.TEST.NAME, " (mg/L)"), 
  ...
  fig = ggplot() +  
    ...
    geom_errorbar(data=tdata%>%filter(TEST==PK.TEST.NAME), aes(x=NTIM, y=Mean, ymin = meanMinusSE, ymax = meanPlusSE), width=2) + 
    ...
    geom_errorbar(data=tdata%>%filter(TEST==PD.TEST.NAME), aes(x=NTIM, y=Mean,ymin = meanMinusSE, ymax = meanPlusSE), width=2) + 
  ...
  # linear scale
  attr(fig, 'title') <-  paste0("Mean(±SE) Concentration of ", 
                                PK.TEST.LABEL, 
                                " in Serum vs Nominal Sampling Day Following Subcutaneous or Intravenous Dose(s) of ", 
                                PK.TEST.NAME, " by Quantile of Concentration at Each Time Point (", STUDY.NAME, ")")
