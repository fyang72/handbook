...
check.nmdat.venn.diagram <- function(nmdat, values4nmdat)  {
  
 validate(need(nmdat, message=FALSE), 
          need(all(c("ID", "EVID", "AMT", "TEST") %in% colnames(nmdat)), message=FALSE)
 )
  ...
  adpc = nmdat %>% filter(!is.na(as_numeric(ID)), EVID==0, TEST == "REGN3918" )  # unique(TEST)[1])PK.TEST.NAME
...
# ggplot_container
  renderUI({
    output$nmdat.venn.diagram8 <- renderPlot({
       values4nmdat$FIGURE$nmdat.venn.diagram
    })
...
check.NTIM.TIME <- function(nmdat, values4nmdat) {
     validate(need(nmdat, message='FALSE'), 
   ...
   fig = ggplot(tdata, aes(x=NTIM, y=TIME)) + 
     ...
     geom_point(data=tdata %>% 
                  filter(TIME>0) %>% top_n(20, DIFF) %>% 
                  select(ID, NTIM, TIME, DIFF), 
                color="red") + 
   ...
   renderUI({ 
        callModule(module_ggplot_brush, "advanced.check.NTIM.TIME", 
              fig=values4nmdat$FIGURE$nmdat.NTIM.TIME, 
              mydata = nmdat_inputData(),  
              xvar="NTIM", yvar="TIME")
     
     module_ggplot_brush_UI(id="advanced.check.NTIM.TIME", label = "advanced.check.NTIM.TIME")
...
check.missing.samples.events <- function(nmdat, values4nmdat) {
  validate(need(nmdat, message='FALSE'), 
...
fig = ggplot(tdata%>%filter(EVID==0), aes(x=TIME, y=ID)) + 
 ...
 values4nmdat$FIGURE$missing.samples.events =  fig  
 ...
 return(values4nmdat)
   ...
   renderUI({ 
        callModule(module_ggplot_brush, "ggplot.missing.samples.events", 
              fig=values4nmdat$FIGURE$missing.samples.events, 
              mydata = nmdat_inputData(), #%>%filter(TEST%in%c("REGN3918", "CH50H")), 
              xvar="TIME", yvar="ID")
...
check.planned.dose.actual.dose <- function(nmdat, values4nmdat) {
  validate(need(nmdat, message='FALSE'), 
  ...
  tdata = tdata %>% 
    select(C, ROWID, ID, TIME, ARMA, DOSE_MKG, AMT, RATE, INF_TIME, EVID, EXROUTN, CMT, MDV, WGTBL, EXROUTE, USUBJID)
...
renderUI({
  callModule(module_table_output, "planned.dose.actual.dose",  
               mytab = values4nmdat$TABLE$planned.dose.actual.dose)
