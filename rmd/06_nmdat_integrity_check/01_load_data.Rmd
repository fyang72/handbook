...
```{r} 
  ALL = callModule(module_load_data, "load_nmdat_for_integrity_check", ALL, data_name="check_nmdat")
  
  renderUI({
    fluidRow( 
        column(6,
             module_load_data_UI(id="load_nmdat_for_integrity_check", label="Load data") 
             )
    )
  })
...
renderUI({
  ...
  #print(" ALL$DATA[[ check_nmdat ]]")
  #print( ALL$DATA[["check_nmdat"]])

  #})
    callModule(module_table_output, "nmdat_inputData_3465346",  
               mytab = (ALL$DATA[["check_nmdat"]])   # nmdat_inputData()    
             )
  
  validate(need(ALL$DATA[["check_nmdat"]], message="no data loaded"))
  
  module_table_output_UI(id="nmdat_inputData_3465346", 
                          label="nmdat_inputData" ) 
})
```
