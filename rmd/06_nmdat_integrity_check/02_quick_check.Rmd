...
nmdat_mandatory_check <- function(nmdat, values4nmdat)  {
  ...
  values4nmdat = check.colnames.in.stdlist(nmdat, col.lst=nmdat.mandatory.col.lst) %>% 
...
* `r status ` All mandatory primary variables except C and CFLAG must be integer or numeric, as shown in [Confluence](https://sandboxconfluence.regeneron.com/display/PMx/Data+format). 
    + `r info ` 
...
check.obs.order.by.time <- function(nmdat, values4nmdat)  {
     ...
     if(!status=="green") {values4nmdat$red_alert = rbind(isolate(values4nmdat$red_alert), info)}
     
     # pass_quick_check
     values4nmdat$pass_quick_check = values4nmdat$pass_quick_check & status=="green"
...
yes_or_no <- renderUI(renderText({
       validate(need(values4nmdat$mandatory_check, message=FALSE),
       need(values4nmdat$obs.order.by.time$status %in% c("green", "yellow", "red"), message=FALSE), 
       need(values4nmdat$obs.order.by.time$info, message=FALSE)
  )
  
  ifelse(values4nmdat$obs.order.by.time$status=="green", colFmt(asis_output("Yes\U2714"),'green'),  colFmt(asis_output("No\U2714"),'red'))
  })() %>% HTML())
...
info <- renderUI(renderText({
  ...
  if(!values4nmdat$obs.order.by.time$status=="green") {colFmt(values4nmdat$obs.order.by.time$info,'red')} 
  })() %>% HTML())
...
yes_or_no <- renderUI(renderText({
       validate(need(values4nmdat$mandatory_check, message=FALSE),
       need(values4nmdat$missing.ID$status %in% c("green", "yellow", "red"), message=FALSE), 
       need(values4nmdat$missing.ID$info, message=FALSE)
  )
  
  ifelse(values4nmdat$missing.ID$status=="green", colFmt(asis_output("Yes\U2714"),'green'),  colFmt(asis_output("No\U2714"),'red'))
  })() %>% HTML())
...
info <- renderUI(renderText({
  ...
  if(!values4nmdat$missing.ID$status=="green") {colFmt(values4nmdat$missing.ID$info,'red')} 
  })() %>% HTML())
...
yes_or_no <- renderUI(renderText({
        validate(need(values4nmdat$mandatory_check, message=FALSE),
       need(values4nmdat$missing.TIME$status %in% c("green", "yellow", "red"), message=FALSE), 
       need(values4nmdat$missing.TIME$info, message=FALSE)
  )
  
  ifelse(values4nmdat$missing.TIME$status=="green", colFmt(asis_output("Yes\U2714"),'green'),  colFmt(asis_output("No\U2714"),'red'))
})() %>% HTML())
...
info <- renderUI(renderText({
  ...
  if(!values4nmdat$missing.TIME$status=="green") {colFmt(values4nmdat$missing.TIME$info,'red')} 
})() %>% HTML())
...
yes_or_no <- renderUI(renderText({
       validate(need(values4nmdat$mandatory_check, message=FALSE),
       need(values4nmdat$missing.MDV$status %in% c("green", "yellow", "red"), message=FALSE), 
       need(values4nmdat$missing.MDV$info, message=FALSE)
  )
  
  ifelse(values4nmdat$missing.MDV$status=="green", colFmt(asis_output("Yes\U2714"),'green'),  colFmt(asis_output("No\U2714"),'red'))
  })() %>% HTML())
...
info <- renderUI(renderText({
  ...
  if(!values4nmdat$missing.MDV$status=="green") {colFmt(values4nmdat$missing.MDV$info,'red')} 
  })() %>% HTML())
...
yes_or_no <- renderUI(renderText({
        validate(need(values4nmdat$mandatory_check, message=FALSE),
       need(values4nmdat$missing.CMT$status %in% c("green", "yellow", "red"), message=FALSE), 
       need(values4nmdat$missing.CMT$info, message=FALSE)
  )
  
  ifelse(values4nmdat$missing.CMT$status=="green", colFmt(asis_output("Yes\U2714"),'green'),  colFmt(asis_output("No\U2714"),'red'))
})() %>% HTML())
...
info <- renderUI(renderText({
  ...
  if(!values4nmdat$missing.CMT$status=="green") {colFmt(values4nmdat$missing.CMT$info,'red')} 
})() %>% HTML())
...
yes_or_no <- renderUI(renderText({
        validate(need(values4nmdat$mandatory_check, message=FALSE),
       need(values4nmdat$inconsistent.DV.MDV$status %in% c("green", "yellow", "red"), message=FALSE), 
       need(values4nmdat$inconsistent.DV.MDV$info, message=FALSE)
  )
  
  ifelse(values4nmdat$inconsistent.DV.MDV$status=="green", colFmt(asis_output("Yes\U2714"),'green'),  colFmt(asis_output("No\U2714"),'red'))
})() %>% HTML())
...
info <- renderUI(renderText({
  ...
  if(!values4nmdat$inconsistent.DV.MDV$status=="green") {colFmt(values4nmdat$inconsistent.DV.MDV$info,'red')} 
})() %>% HTML())
