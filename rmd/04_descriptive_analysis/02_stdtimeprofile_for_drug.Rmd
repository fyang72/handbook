...
### Concentration of drug <a name="run_sim"></a>


#### Mean time-profile <a name="run_sim"></a>
```{r}
drug_despcriptive_analysis_inputData <- reactive({
...
output$drug_despcriptive_analysis_population_mean_plot <- renderUI({ 
  # Create a namespace function using the provided id
  #ns <- NS(id)
  
  validate(need(drug_despcriptive_analysis_inputData(), message="no data found yet"))
  
    script = readLines("./script/desp_mean_profile.R")
    script = paste0(script, sep="\n")   # sep="<br/>")
    script = paste0(script, collapse="")
  
   isolate({ 
      ALL = callModule(module_run_script, "drug_despcriptive_analysis_population_mean_plot", 
                       ALL, 
                       dataset=drug_despcriptive_analysis_inputData(), 
                       script=script)
   }) 
  

   module_run_script_UI(("drug_despcriptive_analysis_population_mean_plot"), label = NULL) 
     
})

 fluidRow(column(12, uiOutput(("drug_despcriptive_analysis_population_mean_plot"))))
...
output$drug_despcriptive_analysis_population_indiv_plot <- renderUI({ 
  # Create a namespace function using the provided id
  #ns <- NS(id)
  
  validate(need(drug_despcriptive_analysis_inputData(), message="no data found yet"))
  
    script = readLines("./script/desp_indiv_profile.R")
    script = paste0(script, sep="\n")   # sep="<br/>")
    script = paste0(script, collapse="")
  
   isolate({ 
      ALL = callModule(module_run_script, "drug_despcriptive_analysis_population_indiv_plot", 
                       ALL, 
                       dataset=drug_despcriptive_analysis_inputData(), 
                       script=script)
   }) 
   ...
   module_run_script_UI(("drug_despcriptive_analysis_population_indiv_plot"), label = NULL) 
     
})

 fluidRow(column(12, uiOutput(("drug_despcriptive_analysis_population_indiv_plot"))))
