...
output$despcriptive_time_profile <- renderUI({ 
 ...
 #print( normalizePath(".") )
  ...
  script <- list.files(path=paste0(HOME, "/script/"), 
                         full.names = TRUE,
                         pattern="despcriptive_time_profile")
  names(script) = basename(script)
  
   isolate({ 
      ALL = callModule(module_run_script, "despcriptive_time_profile", 
                       ALL, 
   ...
   module_run_script_UI(("despcriptive_time_profile"), label = NULL) 
     
})

 fluidRow(column(12, uiOutput(("despcriptive_time_profile"))))
