...
server <- function(input, output, session) {
  output$network <- renderVisNetwork({
    visNetwork(vis.nodes, vis.links, width="100%", height="800px", #background="#eeefff",
  ...
  output$shiny_return <- renderPrint({
    ...
    #print(links)
    visNetworkProxy("network") %>% visGetSelectedNodes()
     # links %>% filter(from %in% input$network_selectedNodes )
      
      links %>% filter(from %in% input$click )
