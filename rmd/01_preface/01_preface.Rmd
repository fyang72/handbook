...
This handbook is for pharmacometrician to perform pharmacomatrics analysis, from data standardization to report generation. 
Shiny app provides tremendous opportunities to facilitate the routine analysis which could easily occupy the workload for a pharmacometrician. This handbook presents a semi-automation approach to streamline the process using Shiny+Rmarkdown interactive applications, from data standardization, time-profile plot, pkpd analysis, submit jobs to HPC cluster, post-process the fetched result, run simulation, and generate preliminary word-based reports and powerpoint slides for reviewing.  Such a process not only greatly increased the productivity and reproducibility of the deliverables, also promoted the knowledge-sharing across the different shareholders. 
  
## How to read this book

Each chapter is relatively independent. Typically, it will ask for an input dataset, a model or a set of parameters, then perform the corresponding analyses, with the output results presented in the last.


## Key R packages and conventions   

Here is a list of key packages used in the app. 
