...
Two approaches were presented in this handbook,     

1. construct the dataset from **scratch**, i.e. adsl, adex, adpc, etc. see section [Assemble nmdat](#assemble_nmdat); 
    ...
    + merge all these datasets,  see section [assemble](#assemble_all); 
    
2. standardize a preliminary dataset, see section [Standardize nmdat](#standardize_nmdat); 
...
output$standardize_adex <-renderUI({
  ...
  script = readLines(paste0(HOME, "/script/build_adex.R"))
  script = paste0(script, sep="\n")   # sep="<br/>")
  script = paste0(script, collapse="")
...
output$standardize_adpc <-renderUI({
  ...
  script = readLines(paste0(HOME, "/script/build_adpc.R"))
  script = paste0(script, sep="\n")   # sep="<br/>")
  script = paste0(script, collapse="")
...
### assemble  <a name="assemble_all"></a>
...
default_adpx_checkin <- reactive({ 
  read_excel(paste0(HOME, "/lib/pkmeta.xlsx"),sheet="adpx",col_names = TRUE)  %>% 
    as.data.frame() %>% filter(!is.na(standard.name))
})
...
output$standardize_adpx <-renderUI({
  ...
  script = readLines(paste0(HOME, "/script/build_adpx.R"))
  script = paste0(script, sep="\n")   # sep="<br/>")
  script = paste0(script, collapse="")
...
output$standardize_nmdat <-renderUI({
  ...
  script = readLines(paste0(HOME, "/script/build_nmdat.R"))
  script = paste0(script, sep="\n")   # sep="<br/>")
  script = paste0(script, collapse="")
