...
adsl0 <- read_csv("~/handbook/data/adsl.csv", col_names=TRUE,  
         col_type=cols(.default=col_character()))  # read as character  )

adsl <- adsl0 %>% build_adsl() 
check_adsl(adsl0, adsl)
   
adex0 <- read_csv("~/handbook/data/adex.csv", col_names=TRUE,  
               col_type=cols(.default=col_character()))  # read as character  )

adex <- adex0 %>% build_adex()
check_adex(adex0, adex)
 


adpc0 <- read_csv("~/handbook/data/adpc.csv", col_names=TRUE,  
                 col_type=cols(.default=col_character()))  # read as character  )

adpc <- adpc0 %>% build_adpc()
check_adpc(adpc0, adpc)


adsl$dgddg = 9
adex$adex646 = 45
adpc$adpc65356 = 43643
adpx <- build_adpx(adsl, adex, adpc, other=NULL)

nmdat <- adpx %>% build_nmdat()
