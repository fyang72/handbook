...
user.name = tolower(Sys.info()["user"])  # determineCurrentUser()
 ...
 select_which_program_container <- renderUI({
 ...
 select_which_run_container <- renderUI({
  ...
  fluidRow(
    column(width=6, uiOutput(("select_which_program_container"))), 
    
    column(width=6, uiOutput(("select_which_run_container")))
  ) 
...
```{r}
renderUI({
  ALL = callModule(load_cppModel, "load_cppModel_for_integrity_check", ALL)
  
  load_cppModel_Input(id="load_cppModel_for_integrity_check", label="load_cppModel") 
  })
...
return(values4nmdat)
 ...
 renderUI({ 
    ...
    callModule(ggplot_brush, "ggplot.newuoa.timeProfile", 
              fig=  values4nmdat$newuoa$timeProfile , 
              mydata = values4nmdat$newuoa$simData, 
              xvar="TIME", yvar="PRED")
     
    ggplot_brush_Input(id="ggplot.newuoa.timeProfile", label = "ggplot.newuoa.timeProfile")
...
#  
  renderUI({ 
    ...
    callModule(ggplot_brush, "ggplot.newuoa.diagnostic.PRED.DVOR", 
             fig=values4nmdat$newuoa$diagnostic_PRED_DVOR, 
             mydata = values4nmdat$newuoa$simData, 
             xvar="PRED", yvar="DV")
    
    ggplot_brush_Input(id="ggplot.newuoa.diagnostic.PRED.DVOR", 
...
#nmdat_inputData()
 ...
 observeEvent({cppModel_inputData()}, {
