...
indivProfileUI <- function(id, label="") {
  ...
  fluidRow(
    column(4, 
           fluidRow(
             box(width=12, title="Data and filter", collapsible = TRUE, collapsed = FALSE,  solidHeader = TRUE, status =  "primary" , 
                 column(6,
             ...
             box(width=12, title="X-axis and Y-axis", collapsible = TRUE, collapsed = FALSE,  solidHeader = TRUE, status =  "primary" , 
                 column(6, 
    ...
    column(8, 
           fluidRow(
             box(width=12, title="Simulated individual profiles:", collapsible = TRUE, collapsed = FALSE,  solidHeader = TRUE, status =  "primary" ,
...
indivProfile <- function(input, output, session, DATA, FIGURE_ALL) {
  ...
  filterData<- reactive({
    ...
    if (is.null(input$dose_group)) {return(NULL)}
    
    data = inputData() 
    if (input$test!="NULL") {data = data %>% filter(TEST %in% input$test) }
    if (input$dose_group!="NULL") {data = data %>% filter(ARMA %in% input$dose_group) } 
  ...
  output$test_Selector <- renderUI({
    ...
    test.lst = c(unique(data$TEST), "NULL")
  ...
  output$xlim_selector <- renderUI({
    ...
    if (input$test!="NULL") {data = data %>% filter(TEST %in% input$test) } ######### isolate
    
    xvar= data %>% pull(input$xvar) %>% as_numeric()
    xvar = xvar[which(!xvar %in% c(NA, NaN, Inf, -Inf))]
    
    max   <- max(xvar, na.rm=TRUE)
    min   <- min(xvar, na.rm=TRUE) 
    tick = (max - min)/20 %>% round(., digits=1) 
    
    cat(file=stderr(), "##############Step: Xrange2 #################", "\n")
    
    sliderInput(ns("xRange"), paste("Range of ", input$xvar), 
                min = floor(min), max = ceiling(round(max,digits=3)), value = c(min,max), step = tick, ticks = TRUE)
  })
  ...
  output$ytick_selector <- renderUI({
    ...
    if (input$test!="NULL") {yvar = data %>% filter(TEST %in% input$test) %>% pull(input$yvar) %>% as_numeric()}
    if (input$test=="NULL") {yvar = data %>% pull(input$yvar) %>% as_numeric()}
  ...
  figLog <- reactive({
    ...
    if (!is.null(input$data2_name)) { 
      if (input$data2_name!="NONE" & 
        ...
        data2 = DATA$mDATA[[input$data2_name]]
        if (input$test2!="NULL") { data2 = data2 %>% filter(TEST %in% input$test2) }
