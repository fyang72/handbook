...
module_save_multiple_figures_UI <- function(id, label = "") {
  ...
  tagList(
    ...
    fluidRow(
      column(2,  
             actionButton(ns("update_all_figures"),label="Refresh figures", style=actionButton_style)
      ),
...
ns <- session$ns 
values <- reactiveValues(figures=figures)
   
output$checkboxGroup_input <- renderUI({  
  shiny::validate(need(figures, message="no figure found yet"))
  
  checkboxGroupInput(ns("figure_list"), "Select figure(s):",
                     inline=TRUE, 
                     choices = names(figures), 
                     selected = names(figures)
                     )
...
output$multiple_figures_container <- renderUI({  
  validate(need(is.list(figures), message="no figure found, or output$figure needs to be a list")#, 
           #need(input$figure_list, message=FALSE)
  )
    
  subset_of_figures <- values$figures  #figures[input$figure_list]
  
    lapply(1:length((subset_of_figures)), function(i) {
      validate(need(subset_of_figures[[i]], message="no figure found"), 
               need(is.ggplot(subset_of_figures[[i]]), message="only ggpot object allowed")
      )
      
      # save figure into ALL$FIGURE
      ALL = callModule(module_save_figure, paste0("module_save_figure_", i), 
                       ALL, 
                       figure = subset_of_figures[[i]], 
                       figure_index = i, 
...
# update_all_figures
#------------------
observeEvent({input$update_all_figures}, {
  validate(need(length(figures), message="no figure found"), 
           need(input$figure_list, message=FALSE)
  )
   
   values$figures = figures[input$figure_list]
    
   showNotification("all figures updated", type="message") 
...
observeEvent({input$save_all_figures}, {
  validate(need(length(values$figures), message="no figure found") )
  isolate({ 
    
    #lapply(1:length(values$figure), function(i) ALL$FIGURE[[names(values$figure)[i]]] = values$figure[[i]])
    ALL$FIGURE = c(ALL$FIGURE, values$figures)
...
output$docx_all_figures <- downloadHandler(
  ...
  content = function(file) {
    ...
    mydocx <- read_docx(paste0(HOME, "/lib/docTemplate.docx"))  %>% 
      print2docx(FIGURE=values$figures)
...
output$pptx_all_figures <- downloadHandler(
  ...
  content = function(file) {
    ...
    mypptx <- mypptx %>% print2pptx(FIGURE=values$figures)
