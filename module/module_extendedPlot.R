...
#-----------------------------------------
# note: 1) adapted from popkinr, by Patrick Nolain
module_extendedPlot_UI <- function(id, title = "plot", exportable = TRUE, zoomable = TRUE, verbose = FALSE,
...
module_extendedPlot <- function(input, output, session,
                         reactive_run,
                         reactive_plot = NULL,
                         plot_extra = NULL,
                         r_code = TRUE){
  ...
  output$download_plot <- downloadHandler( filename = function() {
    file <- "strtssfsf" #sprintf("%s-%s.%s", inner_run()$info$run_id, input$filename, input$file_type)
    file
